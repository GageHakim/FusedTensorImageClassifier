...
def prepare_model(ckpt_path, input_dir):
    make_deterministic()
    device = utils.get_device()  # Changed to use utils.get_device() for consistency
...
def compress_and_save(model, args, data_loader, output_dir):
    ...
    with torch.no_grad():
        for idx, (data, bpp, filenames) in enumerate(tqdm(data_loader), 0):
            ...
            model.logger.info(
                f"Saved {out_path}. Attained: {actual_bpp:.3f} bpp vs. theoretical: {theoretical_bpp:.3f} bpp."
            )
...
def main(**kwargs):
    ...
    parser.add_argument(
        "--compress_only",
        help="Only compress the images to .hfc format and exit.",
        action="store_true",
    )
    parser.add_argument(
        "--decompress_only",
        help="Only decompress .hfc files from image_dir to png.",
        action="store_true",
    )
    ...
    # LOGIC CHECK
    if args.compress_only and args.decompress_only:
        raise ValueError("Cannot specify both --compress_only and --decompress_only.")

    # DISPATCH LOGIC
    if args.compress_only:
        print("Running in Compress Only Mode...")
        # FIX: Create output directory explicitly before setting up logger in prepare_model
        utils.makedirs(args.output_dir)

        model, loaded_args = prepare_model(args.ckpt_path, args.output_dir)

        # Update loaded args with command line args regarding directories
        loaded_args.image_dir = args.image_dir
        loaded_args.normalize_input_image = True

        loader = prepare_dataloader(loaded_args, args.image_dir, args.output_dir)
        compress_and_save(model, loaded_args, loader, args.output_dir)

    elif args.decompress_only:
        print("Running in Decompress Only Mode...")
        # FIX: Create output directory explicitly before setting up logger in prepare_model
        utils.makedirs(args.output_dir)
        ...
        # Locate HFC files
        hfc_files = glob.glob(os.path.join(args.image_dir, "*.hfc"))
        if not hfc_files:
            print(f"No .hfc files found in {args.image_dir}")
            return

        for hfc_path in tqdm(hfc_files):
            filename = os.path.basename(hfc_path).replace("_compressed.hfc", "")
            out_path = os.path.join(args.output_dir, f"{filename}_reconstructed.png")
            load_and_decompress(model, hfc_path, out_path)
